...
# -*- coding: utf-8 -*-
#
# Audi DIS Media Player Display V4.5 (Client App - Buffer Fix)
#
# - Fix: Sends Commits in small batches, to prevent buffer overflow  (49 Bytes).
#
import zmq
import json
import time
...
class DisplayManager:
    ...
    # These are placeholders. Use `candump can0` to find your real values. (Usually 0x5C3)
    ...
    def draw_music_screen(self):
        ...
        # FIX V4.5: seperaste into smaller frames to avoid buffer overflow
    ...
    def draw_nav_screen(self):
        ...
        # FIX V4.5 seperate into smaller frames to avoid buffer overflow
    ...
    def run(self):
        while True:
            try:
                ...
                # FIX V4.5: 'commit' *insude* draw_xxx_screen
                # self.draw_socket.send_json({'command': 'commit'})
...
if __name__ == "__main__":
    ...
    dm = DisplayManager(
        ...
        config_path='/home/pi/config.json' # make sure it's your path
    )
    dm.initialize()
    dm.run()